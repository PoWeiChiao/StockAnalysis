...
def dca() -> None:
    ...
    """
    codes = ['TQQQ', 'QQQ', 'UPRO', 'VOO', 'NVDA', 'TSM', 'AAPL', 'MSFT']
    start_time = datetime(2022, 1, 1)
    holding_time = timedelta(days=365)
    ...
    for code in codes:
        ...
        print(f'holding {info.get_share()} {code} in {round(info.get_average(), 2)}, '
              f'profit: {round(info.get_profit_in_percentage(current_time), 2)}%')
        print(f'current price: {round(stockCenter.get_stock_price(code, current_time), 2)} '
              f'in {current_time.strftime("%Y-%m-%d")}')
